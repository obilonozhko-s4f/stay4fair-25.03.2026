 ...
 * - [NEW]: Добавлена загрузка аватара (multipart/form-data).
 * - [FIX]: Установлена жесткая защита от автозаполнения паролей браузерами.
 * - [UX]: Добавлен переключатель видимости пароля (Глазик).
 */
final class OwnerProfileProvider
{
    ...
    public function renderProfile(): string
    {
        ...
        <style>
            ...
            /* RU: Стили для обертки пароля с глазиком */
        ...
        <div class="sf-profile-wrap">
            ...
            <?php if (isset($_GET['delete_error']) && $_GET['delete_error'] === 'active_bookings'): ?>
                <div class="sf-alert sf-alert-danger"><strong>⚠️ Aktion blockiert!</strong><br>Ihre Apartments wurden vom Netz genommen (Offline). Eine vollständige Löschung des Accounts ist jedoch nicht möglich, da Sie noch aktive zukünftige Buchungen haben. Bitte kontaktieren Sie unseren Support.</div>
            <?php endif; ?>
        ...
        <script>
            // RU: Функция переключения видимости пароля (Глазик)
            function sfTogglePassword(inputId, iconSpan) {
                const input = document.getElementById(inputId);
                if (input.type === 'password') {
                    input.type = 'text';
                    iconSpan.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>'; // Закрытый глаз
                } else {
                    input.type = 'password';
                    iconSpan.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>'; // Открытый глаз
                }
            }

            // RU: Валидация совпадения паролей
