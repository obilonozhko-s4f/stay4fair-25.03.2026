...
/**
 * Version: 1.3.1
 * RU: Генератор Ваучеров. Кнопка отмены перенесена в конец после политик.
 * EN: Voucher Generator. Cancellation button moved to the end after policies.
 */
final class VoucherGenerator
{
    ...
    /**
     * RU: Получить номер ваучера.
     * EN: Get voucher number.
     */
    ...
    public static function renderHtml(int $bookingId): string
    {
        ...
        if (function_exists('MPHB')) {
            try {
                ...
                if ($booking) {
                    $guestEmail = $booking->getCustomer()->getEmail();
        ...
        // =========================================================
        // 4. CANCELLATION LINK GENERATION
        // =========================================================
        $cancelManager = new CancellationManager();
        $cancelToken = $cancelManager->generateToken($bookingId, $guestEmail);
        $secureCancelLink = add_query_arg(['bid' => $bookingId, 'e' => $guestEmail, 'token' => $cancelToken], site_url('/manage-booking/'));
        ...
        <body>
            ...
            <div class="box" style="background-color: #f9f9f9; text-align: center; border: 1px dashed #000;">
                ...
                <a href="<?php echo esc_url($secureCancelLink); ?>" class="cancel-btn">Manage / Cancel Booking</a>
            </div>
