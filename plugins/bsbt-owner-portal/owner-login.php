 ...
 * Description: Version 2.9.0 – Added debug codes for invalid token errors.
 * Version: 2.9.0
...
final class BSBT_Owner_Login_System {
    ...
    public function render_login_form() {
        ...
        // RU: Чтение ошибки с Debug-кодом (очень поможет, если баг повторится)
        if ( isset($_GET['sf_error']) && $_GET['sf_error'] === 'invalid_token' ) {
            $reasonCode = isset($_GET['reason']) ? ' (Code: ' . sanitize_text_field($_GET['reason']) . ')' : '';
        ...
        if ( isset($_SERVER['REQUEST_METHOD']) && strtoupper((string)$_SERVER['REQUEST_METHOD']) === 'POST' && isset($_POST['bsbt_login_submit']) ) {

            if ( !isset($_POST['bsbt_owner_login_nonce']) || !wp_verify_nonce($_POST['bsbt_owner_login_nonce'], 'bsbt_owner_login_action') ) {
        ...
        <div class="bsbt-login-page-wrapper">
            <div class="bsbt-login-card">
                ...
                <?php if ( $success_msg ) : ?><div class="bsbt-success-box"><?= esc_html($success_msg) ?></div><?php endif; ?>
