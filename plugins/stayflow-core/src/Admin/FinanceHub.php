...
/**
 * Version: 3.9.1
 * RU: Центр Финансов. Исправлен баг поиска по датам (MPHB сохраняет даты без нижнего подчеркивания).
 * EN: Finance Hub. Fixed date search bug (MPHB stores dates without underscore).
 */
final class FinanceHub
{
    ...
    private function renderPayoutsTable(): void
    {
        ...
        // EN: Fixed meta_query. Searches both with and without underscore.
        $args = [
        ...
        foreach ($bookings as $b) {
            ...
            $payout_data[] = [
                'id' => $b_id, 'edit_url' => $edit_url, 'ext_id' => $ext_id, 'status' => $status,
    ...
    private function renderCityTaxTable(): void
    {
        ...
        // RU: Исправленный meta_query (OR логика для дат)
        $args = [
            'post_type'      => 'mphb_booking', 
